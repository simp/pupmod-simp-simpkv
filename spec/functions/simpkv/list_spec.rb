...
def prepopulate_key_files(root_dir, keydir)
  ...
  data_info.each do |description, info|
    next if info.key?(:skip) || info.key?(:deserialized_value)

    filename = File.join(actual_keydir, description.tr(' ', '_'))
...
describe 'simpkv::list' do
  ...
  context 'basic operation' do
    it 'retrieves key list from a specific backend in options when keys exist' do
    ...
    it 'retrieves key list from the default backend in options when keys exist and app_id unspecified' do
    ...
    it 'retrieves key list from the default backend for the app_id when keys exist' do
    ...
    it 'retrieves key list from the auto-default backend when keys exist and backend config missing' do
    ...
    it 'returns an empty key list when no keys exist but the directory exists' do
    ...
    it 'fails when the directory does not exist and `softfail` is false' do
      is_expected.to run.with_params(keydir, @options_default)
                        .and_raise_error(RuntimeError, %r{simpkv Error for simpkv::list with keydir='#{keydir}'})
    end

    it 'uses global keydir when global config is set' do
    ...
    it 'fails when backend list fails and `softfail` is false' do
      is_expected.to run.with_params(keydir, @options_failer)
                        .and_raise_error(RuntimeError, %r{simpkv Error for simpkv::list with keydir='#{keydir}'})
    end

    it 'logs warning and return nil when backend list fails and `softfail` is true' do
  ...
  context 'other error cases' do
    ...
    it 'fails when merged simpkv options is invalid' do
      ...
      is_expected.to run.with_params(keydir, bad_options)
        ...
        %r{simpkv Configuration Error for simpkv::list with keydir='#{keydir}'})
    end
  end
end
