...
describe 'simpkv ldap plugin anonymous class' do
  ...
  context '#configure' do
    let(:plugin) { plugin_class.new('ldap/default') }

    it 'succeeds using valid config' do
      local_options = options.merge({ 'backend' => 'default' })
      ...
      expect { plugin.configure(local_options) }.not_to raise_error
    end
    ...
    context 'error cases' do
      ...
      it "fails when options missing 'backends' key" do
        local_options = { 'backend' => 'test' }
        expect { plugin.configure(local_options) }
  ...
  context 'public API' do
    ...
    context 'after configure' do
      before(:each) do
        ...
        expect(plugin).to receive(:verify_ldap_access)
        expect(plugin).to receive(:ensure_instance_tree)
        plugin.configure(local_options)
      end
  ...
  context 'internal methods' do
    let(:plugin) { plugin_class.new('ldap/ldapi') }
    ...
    describe '#verify_ldap_access' do
      let(:plugin2) { plugin_class.new('ldap/ldapi') }
      let(:options2) { options.merge({ 'backend' => 'default' }) }
