...
describe 'simpkv adapter anonymous class' do
  ...
  let(:options_file) { options_base.merge({ 'backend' => 'test_file' }) }
  let(:options_failer) { options_base.merge({ 'backend' => 'test_failer' }) }
  let(:options_failer_ctr) do
    {
      'backend'  => 'test_failer',
      'backends' => {
        'test_failer' => {
  ...
  context 'constructor' do
    ...
    it 'discards a plugin class with malformed Ruby' do
      expect(Puppet).to receive(:warning).with(%r{simpkv plugin from .*malformed_plugin.rb failed to load}).at_least(:once)
  ...
  context 'public API' do
    ...
    let(:plugin) do
      p = adapter.plugin_info['file'][:class].new('other')
      p.configure(options_file)
      p
    end
