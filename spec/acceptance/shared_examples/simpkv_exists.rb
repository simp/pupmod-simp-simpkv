...
shared_examples 'simpkv::exists tests' do |host|
  ...
  context "simpkv::exists operation for keys on #{host}" do
    ...
    it 'calls simpkv::exists for valid keys and return true' do
      set_hiera_and_apply_on(host, hieradata_with_valid_keys, manifest,
        { catch_failures: true })
    end

    it 'calls simpkv::exists for invalid keys and return false' do
  ...
  context "simpkv::exists operation for folders on #{host}" do
    ...
    it 'calls simpkv::exists for valid folders and return true' do
      set_hiera_and_apply_on(host, hieradata_with_valid_folders, manifest,
        { catch_failures: true })
    end

    it 'calls simpkv::exists for invalid folders and return false' do
